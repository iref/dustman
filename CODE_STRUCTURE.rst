...
Declares the runtime dependencies of the python package ``iref-dustman``
...
running ``pip install iref-dustman`` except that
...
A shell-script used by the Makefile target ``make release``, it will parse your library version from ``dustman/version.py``.
